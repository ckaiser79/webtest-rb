...
module Webtest
  ...
  class Testrunner
    ...
    def doRun
      ...
      advice = CreateTestcaseHtmlAdvice.new
      advice.logDir = @logDir
      advice.reportResourceDir = WTAC.instance.config.read('main:html-reports:resource-dir')
      advice.testcaseTemplate = WTAC.instance.config.read('main:html-reports:template-testcase')
      advice.name = File.basename(testcaseName) +  " [" + @executionResult.to_s + "]"
      advice.onReturn
  ...
  class DumpImageOnFailureAdvice
    ...
    def onReturn(result = nil, error = nil)
      BrowserInstanceService.instance.dump 'failed-testcase-last-browser-screenshot'
    end
  ...
  class CreateTestcaseHtmlAdvice

    attr_writer :logDir
    attr_writer :reportResourceDir
    attr_writer :testcaseTemplate
    ...
    def createHtmlReport
      ...
      d['images'] = Array.new
      images = Dir.glob(@logDir + '/*.png')
      images.each do |img|
        r = Hash.new

        fnam = File.basename img
        extn = File.extname img
        name = File.basename fnam, extn
      ...
      templateFile = @testcaseTemplate
      destinationFile = @logDir + '/report.html'
      ...
      FileUtils.mkdir @logDir + '/js'
      FileUtils.mkdir @logDir + '/css'
      FileUtils.cp_r Dir.glob(@reportResourceDir + '/js/*'), @logDir + '/js'
      FileUtils.cp_r Dir.glob(@reportResourceDir + '/css/*'), @logDir + '/css'
    end
